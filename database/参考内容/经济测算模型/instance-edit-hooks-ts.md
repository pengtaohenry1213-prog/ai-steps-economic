...
export const useCalcEngine = (instance: any) => {
  ...
  const patchUpdateData = (
    ...
    // console.log('========== patchUpdateData -> mapItem = ', mapItem);
    if (mapItem) {
      // 与数据源比对,如果不一样,则更新
      ...
      // 更新变化数据池,空数据不会被收集. useChangePool -> data: dataChangedMap
      if (isEqualOriginData(code, field, value)) {
        ...
        // 更新变化数据池 useChangePool() -> setData: setChangeData
        setChangeData(code, field, value);
      }

      // 更新变化数据池状态 useChangePool() -> setState: setChangeState
      setChangeState();
    }
  };
  ...
  // 计算逻辑 从autoCalcData 调用 calculateFormula方法 -> autoCalcData
  const { calculateFormula } = useCalculate({
    addLog,
    formulaMap, // useRelationPool -> data: formulaMap,
    globalConfig, // useGlobalConfig -> data: globalConfig,
    getData, // useDataPool -> data: dataMap,
    patchUpdateData, // useDataPool -> setData: setData
    dateFields, // useColumnsModel -> data: dateFields,
  });
  ...
  */
  const addRelationData = (data: EditorTs.FormulaList) => {
    ...
    */
    ...
    /* 
      C. 合并基础指标: 将 基础指标地图 合并到 公式关系地图 中，确保所有需要的指标都包含在依赖图中。
  ...
  return {
    ...
    loadSheetData,
    addRelationData,
    autoCalcData: calculateFormula, // useCalculate({...})
    patchUpdateData,
    redo,
    undo,
