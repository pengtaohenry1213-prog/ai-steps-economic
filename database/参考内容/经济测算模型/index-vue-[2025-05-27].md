  ...
  */
  ...
  cellClick({ column, row }: any) {
    ...
    if (column.field === 'metricName' && row.metricCode) {
      ...
      const formula = getFormulaMap()[`${row.metricCode}-${field}`];
      // console.log(formula);
...
// 获取所有单元格信息的方法
const getAllCellsInfo = () => {
  const columns = sheetColumn[menu.value.pageCode] || [];
  const tableData = sheetData[menu.value.pageCode] || [];
  const flatCells: Record<string, any> = {};
  const exceptFields = ['isFixeds', 'metricName', 'scale', 'unit'];
  // console.log('Rows = ', tableData.length, ', Columns: ', columns.length);

  tableData.forEach((row,index) => {
    columns.forEach((column: object) => {
      const field = column.field;
      if(index===0){ console.log('field = ',field); }
      const metricCode = row.metricCode;
      if (metricCode && field && field !== 'metricCode') {
        const id = `${metricCode}-${field}`;

        if((window as any).all[metricCode] && !exceptFields.includes(field)) {
          flatCells[id] = {
            value: row[field],
            metricCode,
...
// 根据getAllCellsInfo获取的当前表格所有单元格数据, 构建&获取单元格的关系网
function getCellRelationData() {
  ...
  // 1. 统计每个单元格被哪些单元格依赖
  Object.keys(cellsInfo).forEach(cellId => {
    const formula = formulaMap[cellId];
    if (formula && Array.isArray(formula.calcMarks)) {
      formula.calcMarks.forEach((depId: string) => {
        if (!dependentsMap[depId]) dependentsMap[depId] = [];
        dependentsMap[depId].push(cellId);
      });
    }
  });
  ...
  */
  ...
  Object.keys(cellsInfo).forEach(cellId => {
    ...
    relationData[cellId] = {
      ...cellsInfo[cellId],
      formula,
      dependencies: formula?.calcMarks || [], // 是"我依赖谁"
 ...
 */
 function convertToD3Format(cellRelationData: Record<string, any>, inDegree: Record<string, number>) {
  // 1. 准备节点数据
...
/**
 * 将数据转换为Mermaid.js可用的格式
 */
function convertToMermaidFormat(cellRelationData: Record<string, any>) {
  let mermaidCode = 'graph TD;\n';
  
  Object.entries(cellRelationData).forEach(([id, data]) => {
    const nodeName = data.metricName || id;
    // 添加节点
    mermaidCode += `    ${id}["${nodeName}"];\n`;
    
    // 添加边
    (data.dependencies || []).forEach(depId => {
      mermaidCode += `    ${depId} --> ${id};\n`;
    });
  });
...
/**
 * 将数据转换为Cytoscape.js可用的格式
 */
function convertToCytoscapeFormat(cellRelationData: Record<string, any>, inDegree: Record<string, number>) {
  const elements = {
    nodes: [],
    edges: []
  };
 ...
 */
function topologicalSortCellRelationData(cellRelationData: Record<string, any>) {
  // 1. 构建依赖图
  ...
  Object.keys(graph).forEach(id => inDegree[id] = 0); // 初始化入度, 首先把所有节点的入度初始化为0
  ...
  Object.entries(graph).forEach(([id, deps]) => {
    if(id) {
      ...
      deps.forEach(depId => {
        if (inDegree[depId] !== undefined) { // 防止访问未定义的节点导致错误, 确保只对存在于 inDegree 中的节点增加入度
          inDegree[depId]++;
        }
      });
    }
...
const testAll = () => {
  ...
  if(window.formula && Object.entries(formula).length>0){
    // 克隆 window.formula 然后遍历修改个别属性的值
    const _formula = window.formula;
    let filterFormula = Object.entries(_formula).map(item=>{
      const _item = item[1];
      _item.children = [];
      _item.childrenMap = [];
      _item.curFormula = [];
