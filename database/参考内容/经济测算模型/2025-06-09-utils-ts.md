...
function topologicalSortCellRelationData(
  ...
  Object.keys(cellRelationData).forEach((cellId) => {
    ...
    // 检查无效依赖并输出警告
    (cellRelationData[cellId].dependencies || []).forEach((depId: string) => {
      if (!isSpecialData(depId) && !cellRelationData[depId]) {
        console.warn(`[依赖链断裂] 节点 ${cellId} 的依赖 ${depId} 不存在于 cellRelationData 中`);
      }
    });
    graph[cellId] = dependencies;
  });
  ...
  Object.entries(graph).forEach(([id, deps]) => {
    if (!isSpecialData(id)) {
      deps.forEach((depId) => {
        if (!isSpecialData(depId) && inDegree[depId] !== undefined) {
          inDegree[depId]++;
        }
      });
    }
  ...
  if (result.length !== nonSpecialNodes.length) {
    ...
    cycleNodes.forEach((nodeId) => {
      console.warn(`[疑似环/断链] 节点 ${nodeId} 入度: ${inDegree[nodeId]}, 依赖:`, cellRelationData[nodeId]?.dependencies);
    });
    return {
      success: false,
      order: result,
      cycle: cycleNodes,
      message: `检测到循环依赖或依赖链断裂，涉及节点: ${cycleNodes.join(', ')}`,
    };
  }
...
export async function testAll(
  ...
  console.log('C10000A0115-2028 dependencies:', cellRelationData['C10000A0115-2028']?.dependencies);
}
