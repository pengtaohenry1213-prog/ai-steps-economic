...
const useCalculate = (options: any) => {
  //  getData 实际上是 getDataMap 函数，它来自于 useCalcEngine hook。在 index.vue 中
      ...
      for (const attr in data) {
        if (isDate(attr)) {
          ...
          if (year === targetYear) { // 使用目标年份进行匹配
            ...
            } else if (timeType === 'quarter') { // 累加当前年份的4个季度的值
              // 季度统计：累加4个季度的值
              totalValue += Number(data[attr]) || 0;
              console.log('季度统计 - 累加季度值，当前totalValue:', totalValue);
            } else if (timeType === 'month') { // 累加当前年份的12个月的值
              // 月度统计：累加12个月的值
              totalValue += Number(data[attr]) || 0;
    ...
    initCalcFormulaGroup(formulaMap: any) {
      ...
      function findMaxIndex(arr: any[]) {
        ...
        for (const code of arr) {
          if (!baseGroup.includes(code)) { // 排除marks为空的情况code, 即排除非公式指标
      ...
      function insertFormula(key: string) {
        ...
        if (calcFormulaGroup.includes(key)) { return; } // 如果公式已在计算组中，说明已经处理过，直接返回成功

        const node = formulaMap[key];
        if (!node) { 
          console.warn(`公式 "${key}" 不存在`);
          return false; // 改为返回 false，表示处理失败
        }

        // processing.add(key); // 标记正在处理

        try {
          if (node.formula) { // 如果公式有 formula 属性，则先处理其依赖项（calcMarks）
            // 处理所有依赖项
  ...
  */
  ...
  const calculateFormula = (code: string, attrs: string[]) => {
    ...
    calculate.calcFormulaGroup.forEach((code) => {
      ...
      const { marks, formula, formulaName, metricCode, field } = node;

      // 检查 formula 是否为空
      if (!formula) {
        console.warn(`当前指标：${formulaName}，指标Code：${metricCode}，公式为空`);
        return;
      }
      ...
      try {
        ...
        if (!Array.isArray(marks)) {
          console.warn(`当前指标：${formulaName}，指标Code：${metricCode}，marks 不是数组`);
          return;
        }
        
        marks.forEach((markInfo) => {
          const fieldValue = getValue(markInfo, node);
          if(markInfo?.encode.includes('totalYear-')) {
            console.log('mark.forEach -> getValue(markInfo, node): markInfo = ', markInfo, ', node = ', node);
          }
  ...
  const getCalculatedFormula = (nodeId: string) => {
    ...
    try {
      formulaNode.marks.forEach((markInfo: any) => {
        ...
        // console.log('formulaNode.marks.forEach -> getValue: markInfo = ', markInfo, ', formulaNode = ', formulaNode, ', fieldValue = ', fieldValue);
