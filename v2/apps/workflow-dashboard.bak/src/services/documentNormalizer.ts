...
export interface DocumentSection {
  ...
  level: 1 | 2 | 3
  /** 章节类型 */
  type: 'info' | 'list' | 'table' | 'code' | 'custom' | 'keyValue'
  /** 章节内容 */
...
export function unifiedToProposalContent(unified: UnifiedDocument): Record<string, unknown> {
  // Define the result structure with proper typing
  interface ProposalResult {
    name?: string
    type?: string
    decisionMakers?: string[]
    background?: string
    scope?: { inScope: { P0: string[]; P1: string[] }; outScope: string[] }
    milestones?: string[]
    risks?: Array<{ type: string; description: string; impact: string; countermeasure: string }>
    [key: string]: unknown
  }
      ...
      switch (section.id) {
        case 'basic-info':
          if (section.content.type === 'keyValue') {
            ...
            if (kv['项目类型']) result.type = Array.isArray(kv['项目类型']) ? kv['项目类型'][0] : kv['项目类型']
