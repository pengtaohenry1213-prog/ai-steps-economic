...
// 从 TEAM_MEMBERS 中提取可以作为决策人的成员（PMO, PM, TechLead）
...
function renderJsonContent(doc: ProposalContent): string {
  ...
  if (doc.humanGate) {
    ...
    if (typeof doc.humanGate === 'object') {
      const hg = doc.humanGate as any
      if (hg.pmo && Array.isArray(hg.pmo)) {
        html += `<h3>PMO 检查项</h3><ul>${hg.pmo.map((item: string) => `<li>${item}</li>`).join('')}</ul>`
      }
      
      if (hg.security && Array.isArray(hg.security)) {
        html += `<h3>安全检查项</h3><ul>${hg.security.map((item: string) => `<li>${item}</li>`).join('')}</ul>`
      }
    } else {
      html += `<p>${doc.humanGate}</p>`
    }
...
.json-source::-webkit-scrollbar-thumb {
  background: #555;
  border-radius: 4px;
}

.json-source::-webkit-scrollbar-thumb:hover {
  background: #777;
}
