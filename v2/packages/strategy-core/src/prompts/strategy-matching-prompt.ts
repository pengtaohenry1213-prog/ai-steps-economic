...
import type { MatchingRequest, MatchingResponse } from '../types'
...

{
  ...
  "confidence": 0.85,
  "reasoning": "简要说明匹配理由",
  "judgmentBasis": "为什么选T3而非T4：项目是在新架构上新建系统，而非对原核心系统做零中断升级..."
}
...
4. **推理说明**：简要说明为什么做这个匹配
5. **判断依据**：必须说明为什么选择该策略，而非其他相似策略（如T3 vs T4）

## 关键判断原则

**T3 vs T4 的判断**：
- T3（成熟型新项目）：业务需求明确，在新架构上新建系统，目标是高效标准化交付
- T4（核心系统大升级）：对现有核心生产系统进行架构重构，目标是零中断平稳升级
- **判断优先级**：先看业务需求是否明确，再看是否是核心系统升级。迁移复杂度不作为主要判断依据。
- 典型T4场景：银行核心系统不停机升级、生产环境核心系统重构
- 典型T3场景：基于新技术栈构建新版本平台、业务需求明确的新建系统
 ...
 * 第二阶段：根据基础匹配结果 + 模板内容，生成完整的开发策略文档
 */
export function buildStrategyEnhancementSystemPrompt(): string {
  return `你是「开发策略增强专家」，负责根据策略模板和行业架构，生成完整的项目开发策略文档。

## 你的职责

基于第一阶段匹配结果（策略类型 + 行业），结合详细策略模板和行业架构文档，生成结构化的完整开发策略。
...
- phases 只输出阶段一和阶段二（详细），其他阶段简略描述
- moduleDevModes 输出核心模块类型
- typicalRisks 输出3-5个典型风险
- successCriteria 输出3-5个核心成功指标
- industryAdaptation 根据行业定制化描述`
}

/**
 * 构建策略增强的 user prompt
 */
export function buildStrategyEnhancementUserPrompt(
  strategyId: string,
  strategyName: string,
  industryId: string,
  industryName: string,
 ...
 * 解析策略增强的响应
 */
export function parseStrategyEnhancementResponse(response: string): EnhancedStrategy | null {
  try {
    const jsonStr = extractJson(response)
    const data = JSON.parse(jsonStr)
    ...
    return {
      ...
      notApplicableScenarios: data.notApplicableScenarios || [],
      coreCharacteristics: data.coreCharacteristics || [],
      coreConflict: data.coreConflict || '',
      phases: data.phases || [],
      moduleDevModes: data.moduleDevModes || [],
      keyNotes: data.keyNotes || [],
      recommendedToolChain: data.recommendedToolChain || [],
      typicalRisks: data.typicalRisks || [],
      successCriteria: data.successCriteria || [],
      industryAdaptation: data.industryAdaptation || ''
    }
  } catch (e) {
    console.error('解析策略增强响应失败:', e)
    return null
  }
}
